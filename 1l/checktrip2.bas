...
Begin Dialog FAULTDLG 48,46, 125, 191, "Relay time report"
  ...
  OptionGroup .GROUP_2
    OptionButton 90,131,28,10, ".TXT"
    OptionButton 90,140,28,9, ".CSV"
  ...
  Text 4,48,92,8, "Select backup substation"
...
End Dialog
   ...
   If EquipmentType( PickedHnd ) <> TC_RLYGROUP Then
     Print "Must select a relay group on a line"
     Exit Sub
   End If
'   If InputDialog( FltConnection, BusNameList ) = 0 Then Stop
   ...
   If GetData( BrHnd1, BR_nType, BrType& ) = 0 Then GoTo HasError
   ...
   BrHnd2&   = 0
   CountBr2& = 0
   While GetBusEquipment( BusHnd1, TC_BRANCH, BrHnd2 ) > 0
     ' Must skip the picked branch
     If BrHnd2 = BrHnd1 Then GoTo EndLoop
     ...
     If GetData( BrHnd2, BR_nRlyGrp2Hnd, RlyGrp2& ) <= 0 Then GoTo EndLoop
...
'   Print #1, "L2 = " & FullBusName(BusHnd2) & "-" & FullBusName(Bus3List(RemoteIdx))
   Print #1, "Backup Substation = " & FullBusName(Bus3List(RemoteIdx))
   Print #1, "Fault connection = " & FltConnStr(dlg.Group_1+1)
   Print #1, ""

   Dim RlyGrpHndLst(2) As Long
   RlyGrpHndLst(1) = PickedHnd
   RlyGrpHndLst(2) = RlyGrpHnd2
   NOgrp           = 2
'   Call RelayOut( PickedHnd, StepSize, OutRlyCur, CsvOut )
'   Call RelayOut( RlyGrpHnd2, StepSize, OutRlyCur, CsvOut )
   Call RelayOut( RlyGrpHndLst, NOgrp, StepSize, OutRlyCur, CsvOut )
...
'
Sub RelayOut( RlyGrpHnd() As Long, ByVal NOgrp, ByVal StepSize As Double, _
   ...
   GrpHnd& = RlyGrpHnd(1)
   If GetData( GrpHnd, RG_nBranchHnd, BrHnd1& ) = 0 Then GoTo HasError
...
'   While PickFault( ShowFaultFlag ) > 0
   While ShowFault( ShowFaultFlag, 1, 1, 0, ShowFlagRly ) > 0
    ...
    For Index = 1 To NOgrp
      RelayHnd   = 0
      While GetRelay( RlyGrpHnd(Index), RelayHnd ) > 0
        If HeaderLine$ <> "" Then
...
'   Print table of relay current and time for a range of fault
'
Sub RelayOut_old( ByVal RlyGrpHnd As Long, ByVal StepSize As Double, _
              ByVal printCurr As Long, ByVal printCSV As Long)
   Dim MagArray(16) As Double
   Dim AngArray(16) As Double
   Dim ShowFlagRly(4) As Long
