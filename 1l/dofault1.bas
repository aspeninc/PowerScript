...
'   Dim nIterations As Long
   ...
   nRet = SimuFault( PickedHandle, FltConnection, FltOption, OutageType, OutageList, _
          Rflt, Xflt, ClearPrev )
   Exit Sub
   ' Error handling
   HasError:
...
'
Begin Dialog Dialog_SCAP 31,48, 213, 91, "Confirmation"
  OptionGroup .GROUP_1
    OptionButton 16,36,188,8, "Restore original impedance now. (Output from"
    OptionButton 16,56,188,8, "Save original impedance to c:\temp\dofault.tmp"
  Text 8,8,188,12, "Series Cap data have been modified during iteration."
  Text 8,20,172,8, "Please specify how you want to proceed:"
  Text 28,44,176,8, "Fault | Report command may show incorrect result)"
  OKButton 80,72,52,12
End Dialog
...
Function SimuFault( ByVal  nDevHnd As Long, ByRef FltConnection() As Long, _
  ...
  ' Check dofault.sav. Read original X if confirmed
  On Error GoTo Skip1
  Open "c:\temp\dofault.tmp" For Input As 1
  NCount = 0
  Do While Not EOF(1)
    Line Input #1, SCapX
    NCount = NCount + 1
    XoArray(NCount) = SCapX
  Loop
  Close 1
  nRetCode = GetData( HND_SYS, SY_nNOseriescap, NOScap )
  If NOScap <> NCount Then GoTo Skip1
  nRetCode = MsgBox( "Saved series cap. data found in file c:\temp\dofault.tmp." + Chr(10) + _
                     "Click Yes to restore saved data before starting simulation", 3, "Confirm" )
  If nRetCode = 2 Then Exit Function
  If nRetCode <> 6 Then GoTo Skip1
  Call RestoreXo( XoArray )
Skip1:
  Open "c:\temp\dofault.tmp" For Output As 1
  For IterationCount = 1 To MaxIterations	' Do at most this much iterations
    ...
    ' Retrieve short circuit current
    If GetSCCurrent( HND_SC, MagArray, AngArray, 4 ) = 0 Then GoTo HasError
    ISCm = MagArray(1)

    If IterationCount = 1 Then ISCm_p = 2*ISCm  ' This will make sure we do at least 2 iterations

    'Debug
    'Print #1, "Iteration #" + Str(IterationCount) + " ; Isc = " + Str(ISCm) + "; Isc_m = " + Str(ISCm_p)

    ' Check convergence
    dError = Abs(ISCm/ISCm_p - 1)
    If dError < Tolerance Then Exit For

    ISCm_p = ISCm 'Keep for next iteration

    ' Adjust Series cap impedances
    SCapHandle = 0
    SCapsCount = 0
    While GetEquipment( TC_SCAP, SCapHandle ) > 0
      ...
      If StatusFlag = 1 Then  ' Need to adjust only In service Scap
        ...
        If GetData( SCapHandle, SC_dR, Rcap ) = 0 Then GoTo HasError
        If GetData( SCapHandle, SC_dX0, X0cap ) = 0 Then GoTo HasError
        If GetData( SCapHandle, SC_dR0, R0cap ) = 0 Then GoTo HasError
        ...
        If GetSCCurrent( SCapHandle, MagArray, AngArray, 4 ) > 0 Then 
          ...
          If ( I1 > Ipr ) Then 'MOV conducts, recalculate impedance
            ' In first few iterations the current could be very high.
            ' Put a limit on it per Goldsworthy advice
            If ( I1 > 10*Ipr ) Then I1 = 10*Ipr
            Ipu = I1 / Ipr
        ...
        Else
          ...
          R0c = R0cap
        End If
        If SetData( SCapHandle, SC_dX,  Xc  + 0.2*(Xcap-Xc)   ) = 0 Then GoTo HasError
        If SetData( SCapHandle, SC_dX0, Xc + 0.2*(X0cap-Xc) ) = 0 Then GoTo HasError
        If SetData( SCapHandle, SC_dR,  Rc  + 0.2*(Rcap-Rc)   ) = 0 Then GoTo HasError
        If SetData( SCapHandle, SC_dR0, Rc + 0.2*(R0cap-Rc) ) = 0 Then GoTo HasError
        If PostData( SCapHandle ) = 0 Then GoTo HasError	'Save new impedance

      End If
    Wend ' Next Series Cap
  Next  'Up to iteration limit

  Dim dlg As Dialog_SCap
  Button = 0
  If SCapsCount > 0 Then 
    If IterationCount < MaxIterations Then 
      Print "Done:  nIterations = "; IterationCount
      Button = Dialog( dlg )
      Button = dlg.group_1
    Else
    ...
    If Button = 0 Then Call RestoreXo( XoArray )
  End If
  SimuFault = 1
  Exit Function 
  HasError:
...
Sub RestoreXo( ByRef XoArray() As Double )
  SCapHandle = 0
  SCapsCount = 0
  While GetEquipment( TC_SCAP, SCapHandle ) > 0
    If GetData( SCapHandle, SC_nInService, StatusFlag ) = 0 Then GoTo HasError1
    If StatusFlag = 1 Then
      SCapsCount = SCapsCount + 1
      If SetData( SCapHandle, SC_dX,  XoArray(SCapsCount) ) = 0 Then GoTo HasError1
      If SetData( SCapHandle, SC_dX0, XoArray(SCapsCount) ) = 0 Then GoTo HasError1
      If SetData( SCapHandle, SC_dR,  0.0 ) = 0 Then GoTo HasError1
      If SetData( SCapHandle, SC_dR0, 0.0 ) = 0 Then GoTo HasError1
      If PostData( SCapHandle ) = 0 Then GoTo HasError1
    End If
  Wend
HasError1:
End Sub