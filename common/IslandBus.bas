...
' IslandBus.BAS
'
' Generate report for islanded buses
' Usage:
...
Sub main()
 ...
 Do While Not EOF(1)
  ...
  If InStr( aLine$, "Unable to determine the generator reference angle at " ) > 0 Then
   nIslandedBusCount = nIslandedBusCount + 1
   nStart = 53
   nEnd = InStr( aLine$, "kV." )
   ...
   If findBusByName( BusName, BusVolt, nHnd& ) Then 
    BusHnd = nHnd
   Else
    GoTo HasError
   End If
 ...
 Loop
 Print #2, "=================================================================="
 Print #2, "The total number of islanded buses is " + Str(nIslandedBusCount)
 Print "Found" + Str(nIslandedBusCount) + " islanded buses in the existing report."
 Print "The generated report has been saved to " + OutputFile$  
 Close 1
 Close 2
Exit Sub
' Error handling
