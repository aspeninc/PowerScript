...
'
'
Begin Dialog Dialog_1 33,52, 187, 73, "Tie line report"
  OptionGroup .GROUP_1
...
Sub main()
   ...
   Else
     ...
     Print #1, " "
   End If
   OutStr$ = "__Zone1__Zone2___Line____________________________________________"
   If rCode = 1 Then
     rCode = PrintTTY( OutStr$ )
     rCode = PrintTTY( "" )
   Else
...
Sub AddToList( ByVal lineHnd& )
  ...
  For idx1 = Counts-1 To 1 step -1
   ...
   Call GetData( Bus1Handle, BUS_nZone, Area11& )
   Call GetData( Bus2Handle, BUS_nZone, Area12& )
   Call GetData( lineHnd2, LN_nBus1Hnd, Bus1Handle& )
   Call GetData( lineHnd2, LN_nBus2Hnd, Bus2Handle& )
   Call GetData( Bus1Handle, BUS_nZone, Area21& )
   Call GetData( Bus2Handle, BUS_nZone, Area22& )
   nComp& = Area11 - Area21
